...
**Can I Afford This Home?** (`client/src/pages/public/AffordabilityCheck.tsx`): Model B affordability page at `/afford`. Users paste a property address or listing URL to get a full affordability analysis for that specific property. Flow: (1) search bar accepts address or Zillow/Redfin/Realtor.com URL, (2) backend resolves property via RapidAPI auto-complete → search-buy → detail pipeline (`POST /api/properties/lookup`), (3) property card displays photo/price/beds/baths/sqft, (4) financial inputs (income, debts, down payment, credit score, interest rate) with live recalculation, (5) monthly payment breakdown (P&I, tax, insurance, PMI, HOA), (6) DTI qualification check with status badge (affordable/stretch/over budget), (7) "Start Application" CTA pre-fills `/apply` with property price and state. Zero-price guard prevents NaN in calculations. Address matching uses normalized comparison to avoid returning wrong property. Env: `RAPIDAPI_KEY` required for property lookup.
