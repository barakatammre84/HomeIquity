...
**AI Homebuyer Coach** (server/services/coachingService.ts, server/routes/coach.ts): Gemini-powered conversational mortgage readiness advisor. Requires authentication. Fetches user's verified application data (income, credit score, employment, debts, DTI, documents) and passes it as priority context to the AI. System prompt instructs AI to use verified data as ground truth over chat-reported info. Assesses readiness tier (ready_now/almost_ready/building/exploring), creates personalized action plans, and generates tailored document checklists based on employment type. DB tables: coach_conversations, coach_messages. API: POST /api/coach/message, GET /api/coach/conversations, GET /api/coach/profile, GET /api/coach/intake/latest (extracts structured intake data for pre-filling applications). UI at /ai-coach route. ReadinessPanel shows "Ready to Apply" CTA when tier is ready_now/almost_ready.

**Coach-to-Application Data Pipeline**: The coach system prompt includes an "intake" field in its structured data schema that captures financial details (annualIncome, creditScore, employmentType, etc.) during conversations. GET /api/coach/intake/latest aggregates these across all conversations, prioritizing the latest values per field. The PreApproval form (client/src/pages/lending/PreApproval.tsx) fetches this intake data and pre-fills empty form fields, with enum validation (employmentType, propertyType, loanPurpose) and numeric string normalization.

**Onboarding Flow** (client/src/components/WhatsNext.tsx): Guided 4-step journey: 1) Meet AI Coach, 2) Browse Properties, 3) Get Pre-Approved, 4) Upload Documents. Step 2 includes "Already found a home? Skip to application" shortcut link. Steps complete based on coach sessions, property browsing (localStorage), applications, and documents.
