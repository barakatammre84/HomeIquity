...
- **PreApproval Form Autosave**: localStorage-based draft persistence (`baranest_preapproval_draft`, `baranest_preapproval_step`). Debounced at 800ms, saves form values and current step. Shows restore banner on return. Cleared on successful submission. Header shows "Saved" indicator with checkmark.
- **Email Capture Modal**: `EmailCaptureModal` component in App.tsx for anonymous users. Triggers after 3+ page views (tracked via `baranest_anon_pageviews`). Backend: POST `/api/email-capture` with dedup and source page tracking. Table: `email_captures`. Rate limited to 5 req/15min.
- **PersonalizedNudges**: Dashboard nudges expanded to show up to 3 items. Includes timeline nudges for `underwriting` and `conditional` application statuses. Deduplicated against WhatsNext actions by href to avoid showing redundant calls-to-action.
- **Property Search Persistence**: Filters (location, type, price range, mode) saved to localStorage (`baranest_property_filters`). Restored on next visit with "last search" banner. 7-day expiry.
