...
- **PreApproval Form Autosave**: localStorage-based draft persistence (`baranest_preapproval_draft`, `baranest_preapproval_step`). Debounced at 800ms, saves form values and current step. Shows restore banner on return. Cleared on successful submission.
- **Email Capture Modal**: `EmailCaptureModal` component in App.tsx for anonymous users. Triggers after 3+ page views (tracked via `baranest_anon_pageviews`). Backend: POST `/api/email-capture` with dedup. Table: `email_captures`.
- **PersonalizedNudges**: Dashboard nudges expanded to show up to 3 items. Includes timeline nudges for `underwriting` and `conditional` application statuses.
