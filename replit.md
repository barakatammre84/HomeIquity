...
- **CombinedNextSteps**: Unified dashboard component merging WhatsNext actions and PersonalizedNudges into a single section. Primary action prominently displayed, secondary actions in compact 2-column grid. Deduplicates by href. Inline in Dashboard.tsx.
- **Dashboard Consolidation**: Status hub unifies approval status + journey tracker + loan snapshot into single card. Loan snapshot is inline 4-column row with ShieldCheck verified-data lock indicators. Personalized greeting with dynamic subtitles based on homeownership goal (GET /api/homeownership-goal). Total visible cards reduced from ~12 to ~9.
